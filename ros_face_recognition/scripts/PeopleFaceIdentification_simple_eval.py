...
class PeopleFaceIdentificationSimple():
        ...
        if os.path.exists(path):
            ...
            for file in fileList:
                ...
                if(file!='.gitkeep'):
                    ...
                    face_recognition_list=face_recognition.face_encodings(face_image,known_face_locations=[[0,face_image.shape[0],face_image.shape[1],0]])
    ...
    def _publishRosMsg(self,data,data_result,detected_faces_list):
	    ...
	    #detected_face.header.stamp = rospy.get_rostime()
            ...
            x0,y0=self._processBoxCenter(face.top,face.right,face.bottom,face.left)
    ...
    def _processDetectFace(self,top, right, bottom, left,face_encoding):
        ...
        for label in self.faceList.keys():
            match = face_recognition.compare_faces([self.faceList[label].encoding], face_encoding)##classification
    ...
    def detectFaceFromImgSrvCallback(self,req):
        ...
        try:
            ...
            for  face in detected_faces_list:
                ...
                x0,y0=self._processBoxCenter(face.top,face.right,face.bottom,face.left)#ruthrashnono
                detected_face.pose.x=x0
                detected_face.pose.y=y0
                box=Box()
                box.x=face.top
